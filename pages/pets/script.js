...
const cards_wrapp = document.querySelector('.cards_wrapper');
...
cards_wrapp.addEventListener('click', modalWindowOpen);
...
function modalWindowOpen(event) {
    ...
    console.log(event.target.closest('.pets_card'));
    if (event.target.closest('.pets_card')) {
        modalRenderingFunction(event.target.closest('.pets_card').id);
        console.log(event.target.closest('.pets_card').id);
